...
import { DOCUMENT_CHANNELS } from "../../shared/ipcChannels";
...
export const useDocumentDetection = () => {
  ...
  const detectAndShowDocuments = useCallback(
    async (responseText: string): Promise<ResolvedDocument[]> => {
      try {
        console.log(
          "🔍 [DocumentDetection] Analyzing AI response for document references..."
        );

        // First, detect documents locally
        const detectedDocuments =
          DocumentDetector.detectDocuments(responseText);
        ...
        console.log(
          ...
          "potential document references"
        );

        // Try to resolve documents via IPC
        const result = await ipcRenderer.invoke(
          DOCUMENT_CHANNELS.DETECT_AND_RESOLVE_DOCUMENTS,
          responseText
        );

        if (!result.success) {
          console.warn(
            "🔍 [DocumentDetection] Failed to resolve documents:",
            result.error
          );
          return [];
        }
        ...
        console.log(
          "🔍 [DocumentDetection] Successfully resolved",
        ...
        if (resolvedDocuments.length > 0) {
          const bestDocument = resolvedDocuments.reduce(
            (best: ResolvedDocument, current: ResolvedDocument) => {
          ...
          console.log(
            "🔍 [DocumentDetection] Auto-showing document:",
            bestDocument.name
          );
        ...
        console.error(
          "🔍 [DocumentDetection] Error detecting documents:",
          error
        );
        return [];
      }
  ...
  const resolveDocumentPath = useCallback(
    async (documentPath: string): Promise<ResolvedDocument | null> => {
      try {
        console.log(
          "🔍 [DocumentDetection] Resolving document path:",
          documentPath
        );
        ...
        if (!result.success) {
          console.warn(
            "🔍 [DocumentDetection] Failed to resolve document path:",
            result.error
          );
          return null;
        }
        ...
        console.error(
          "🔍 [DocumentDetection] Error resolving document path:",
          error
        );
        return null;
      }
  ...
  const showResolvedDocument = useCallback(
    (document: ResolvedDocument) => {
      console.log(
        "🔍 [DocumentDetection] Showing resolved document:",
        document.name
      );
