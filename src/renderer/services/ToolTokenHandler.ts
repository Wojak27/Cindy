...
export class ToolTokenHandler {
  ...
  private readonly TOKEN_REGEX = new RegExp(
    `${this.TOOL_START_TOKEN}|${this.TOOL_END_TOKEN}|${this.ERROR_START_TOKEN}|${this.ERROR_END_TOKEN}`,
    "g"
  );
  ...
  public processChunk(chunk: string, conversationId: string): ProcessedContent {
    ...
    while ((match = this.TOKEN_REGEX.exec(chunk)) !== null) {
      ...
      if (token === this.TOOL_START_TOKEN || token === this.ERROR_START_TOKEN) {
        ...
        token === this.TOOL_END_TOKEN ||
        token === this.ERROR_END_TOKEN
      ) {
        ...
        if (this.toolStack.length > 0) {
          ...
          if (toolCall) {
            ...
            const startToken = isError
              ? this.ERROR_START_TOKEN
              : this.TOOL_START_TOKEN;
  ...
  private parseToolCall(
    ...
    try {
      ...
      console.debug(
        "[ToolTokenHandler] Raw incoming content chunk:",
      ...
      console.debug(
        "[ToolTokenHandler] Cleaned content chunk:",
    ...
    this.toolStack.forEach((toolContent, index) => {
      if (toolContent.trim()) {
        try {
          ...
          const partialToolCall = this.parsePartialToolCall(
            toolContent,
            conversationId,
            index
          );
  ...
  private parsePartialToolCall(
    ...
    try {
      ...
      const parametersMatch = cleanContent.match(
        /"parameters"\s*:\s*(\{[^}]*\}?)/
      );
      if (parametersMatch) {
        try {
