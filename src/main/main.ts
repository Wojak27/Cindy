...
import {
  ...
  DuckDBVectorStore,
} from "./services/DuckDBVectorStore.ts";
...
const setupSettingsIPC = () => {
  ...
  ipcMain.handle(IPC_CHANNELS.GET_SETTINGS_SERVICE, () => {
    console.log(
      "🔧 DEBUG: Settings service requested by renderer, available:",
      !!settingsService
    );
  ...
  ipcMain.handle(
    ...
    async (event, section: string, value: any) => {
      if (!settingsService) {
        throw new Error("SettingsService not initialized");
      }
  ...
  ipcMain.handle(
    IPC_CHANNELS.SETTINGS_SET_ALL,
    async (event, settings: any) => {
      if (!settingsService) {
        throw new Error("SettingsService not initialized");
      }
      console.log(
        "🔧 DEBUG: settings-set-all called with:",
  ...
  ipcMain.handle(
    IPC_CHANNELS.WAKE_WORD_UPDATE_KEYWORD,
    async (_, keyword: string, sensitivity: number) => {
      console.log(
        "Main process - wake-word:update-keyword IPC called:",
        keyword,
        sensitivity
      );
      try {
  ...
  ipcMain.handle(IPC_CHANNELS.WAKE_WORD_STATUS, async () => {
    ...
    return {
      success: false,
      isListening: false,
      error: "Wake word service disabled",
    };
  });
  ...
  ipcMain.handle(
    IPC_CHANNELS.FETCH_PROVIDER_MODELS,
    async (event, { provider, config }) => {
      console.log(
        `Main process - fetch-provider-models called for provider: ${provider}`
      );
        ...
        switch (provider) {
          ...
          case "ollama":
            try {
              ...
              console.warn(
                "Failed to fetch Ollama models, using defaults:",
                error.message
              );
              models = [];
            }
...
const setupDatabaseIPC = () => {
  ...
  ipcMain.handle(
    IPC_CHANNELS.SHOW_DIRECTORY_DIALOG,
  ...
  ipcMain.handle(IPC_CHANNELS.CREATE_VECTOR_STORE, async (event, options) => {
    ...
    try {
      ...
      if (llmProvider === "ollama") {
        ...
        console.log(
          "[IPC] Using Ollama embeddings with smallest Qwen model (0.5b)"
        );
      } else {
        ...
        if (!apiKey && llmProvider === "auto") {
          ...
          console.log(
            "[IPC] No OpenAI API key found in auto mode, falling back to Ollama embeddings with smallest Qwen model"
          );
          vectorStoreConfig.embeddingProvider = "ollama";
          vectorStoreConfig.embeddingModel = "granite-embedding:278m"; // Smallest Qwen model
        } else if (!apiKey) {
          return {
            success: false,
            message:
              "OpenAI API key required for OpenAI embeddings. Please set your API key or switch to Ollama provider.",
          };
        } else {
      ...
      const vectorStore = await createDuckDBVectorStore(
  ...
  ipcMain.handle(
    IPC_CHANNELS.VECTOR_STORE_INDEX_DIRECTORY,
      ...
      try {
        ...
        if (duckDBVectorStore) {
          ...
          try {
            ...
            duckDBVectorStore.off(
              "indexingCompleted",
              indexingCompletedListener
            );
          }
        }
  ...
  ipcMain.handle(
    IPC_CHANNELS.VECTOR_STORE_CHECK_STATUS,
    async (event, directoryPath) => {
      ...
      try {
        ...
        if (duckDBVectorStore) {
          ...
          const status =
            await duckDBVectorStore.checkDirectoryStatus(directoryPath);
  ...
  ipcMain.handle(
    IPC_CHANNELS.VECTOR_STORE_GET_INDEXED_ITEMS,
    async (event, databasePath) => {
      ...
      try {
        ...
        if (!duckDBVectorStore && databasePath) {
          console.log(
            "[IPC] Vector store not initialized, checking for existing database..."
          );
          ...
          if (fs.existsSync(dbPath)) {
            console.log(
              "[IPC] Found existing vector database, initializing..."
            );
            ...
            if (!duckDBVectorStore) {
              try {
                duckDBVectorStore = await createDuckDBVectorStore(
                  databasePath,
                  vectorStoreConfig,
                ...
                await duckDBVectorStore.initialize();
                console.log(
                  "[IPC] Vector store initialized successfully for reading"
                );
              } catch (error) {
  ...
  ipcMain.handle(
    IPC_CHANNELS.RESOLVE_DOCUMENT_PATH,
    async (event, documentPath: string) => {
      ...
      try {
        ...
        if (duckDBVectorStore) {
          ...
          const matchingFile = indexedFiles.find(
            (file) =>
        ...
        return {
          success: false,
          error: "Document not found in any known location",
        };
      } catch (error) {
  ...
  ipcMain.handle(
    IPC_CHANNELS.DETECT_AND_RESOLVE_DOCUMENTS,
    async (event, responseText: string) => {
      ...
      try {
        ...
        for (const detectedPath of detectedPaths) {
          try {
            ...
            console.warn(
              "[IPC] Failed to resolve detected document:",
              detectedPath,
              resolveError
            );
          }
  ...
  ipcMain.handle(IPC_CHANNELS.READ_FILE_BUFFER, async (event, filePath) => {
    ...
    console.log(
      "[IPC] DuckDB vector store available for file access:",
      !!duckDBVectorStore
    );
    try {
      ...
      if (!isAllowed) {
        return {
          success: false,
          error: "File access not allowed - file must be indexed first",
        };
      }
...
const setupTTSIPC = () => {
  ...
  ipcMain.handle(
    IPC_CHANNELS.TTS_SYNTHESIZE,
    async (event, text: string, outputPath?: string) => {
      console.log(
        "Main process - tts-synthesize IPC called with text:",
        text.substring(0, 50) + "..."
      );
      try {
        ...
        if (!textToSpeechService.isReady()) {
          ...
          try {
            ...
            console.error(
              "Main process - TTS initialization failed:",
              initError
            );
            return {
              success: false,
              error: "Failed to initialize TTS service: " + initError.message,
            };
          }
        }
        ...
        console.log(
          "Main process - tts-synthesize result:",
  ...
  ipcMain.handle(
    IPC_CHANNELS.TTS_SYNTHESIZE_AND_PLAY,
    async (event, text: string) => {
      console.log(
        "Main process - tts-synthesize-and-play IPC called with text:",
        text.substring(0, 50) + "..."
      );
      try {
        if (!textToSpeechService) {
        ...
        console.log(
          "Main process - tts-synthesize-and-play result:",
  ...
  // Update TTS options
  ipcMain.handle(
    IPC_CHANNELS.TTS_UPDATE_OPTIONS,
    async (event, options: any) => {
      console.log(
        "Main process - tts-update-options IPC called with provider:",
        options.provider
      );
      try {
        if (!textToSpeechService) {
          return {
            success: false,
            error: "TextToSpeechService not initialized",
          };
        }
        ...
        if (resolvedOptions.provider === "auto") {
          ...
          console.log(
            "Main process - Auto TTS provider resolved to:",
        ...
        console.log(
          "Main process - TTS options updated successfully with provider:",
          resolvedOptions.provider
        );
        return { success: true };
      } catch (error) {
  ...
  ipcMain.handle(
    IPC_CHANNELS.TTS_REQUEST_MODEL_DOWNLOAD_PERMISSION,
    async (event, request) => {
      console.log(
        "[IPC] TTS model download permission requested:",
        request.modelName
      );
      ...
      try {
        ...
        if (!mainWindow) {
          console.error(
            "[IPC] Main window not available for permission dialog"
          );
        ...
        if (modelPermissions[request.modelName] === "granted") {
          console.log(
            "[IPC] Model download permission already granted:",
            request.modelName
          );
          return { granted: true };
        }
        ...
        if (!modelPermissions[request.modelName]) {
          console.log(
            `[IPC] No explicit permission set for "${request.modelName}", auto-granting download permission for smoother UX`
          );
          return { granted: true };
        }
        ...
        if (modelPermissions[request.modelName] === "denied") {
          console.log(
            "[IPC] Model download permission previously denied:",
            request.modelName
          );
          return {
            granted: false,
        ...
        const result = await dialog.showMessageBox(mainWindow, {
          ...
          detail:
            `Cindy needs to download the text-to-speech model "${request.modelName}" (${request.estimatedSize}) to generate voice output.\n\n` +
            `This will:\n` +
        ...
        if (savePermission && settingsService) {
          ...
          console.log(
            "[IPC] Saved permanent permission for model:",
            request.modelName
          );
        }

        console.log("[IPC] Model download permission result:", {
          granted,
  ...
  ipcMain.on("tts-download-progress", (event, progress) => {
    console.log(
      `[IPC] TTS download progress: ${progress.file} - ${progress.progress}% (${progress.status})`
    );
...
const setupConnectorIPC = () => {
  ...
  ipcMain.handle(IPC_CHANNELS.CONNECTOR_GET_STATUS, async () => {
    try {
      if (!connectorManagerService) {
        return {
          success: false,
          error: "ConnectorManagerService not available",
        };
      }
  ...
  ipcMain.handle(
    IPC_CHANNELS.SETTINGS_GET_OAUTH_CREDENTIALS,
    async (event, provider: string) => {
      try {
        if (!settingsService) {
          return { success: false, error: "Settings service not available" };
        }
        const credentials = await (settingsService as any).getOAuthCredentials(
          provider
        );
        return credentials;
      } catch (error: any) {
        console.error(
          `[IPC] Error getting OAuth credentials for ${provider}:`,
          error
        );
        return null;
      }
  ...
  ipcMain.handle(
    IPC_CHANNELS.SETTINGS_SET_OAUTH_CREDENTIALS,
    async (event, provider: string, clientId: string, clientSecret: string) => {
      try {
        if (!settingsService) {
          return { success: false, error: "Settings service not available" };
        }
        await (settingsService as any).setOAuthCredentials(
          provider,
          clientId,
          clientSecret
        );
        ...
        console.error(
          `[IPC] Error setting OAuth credentials for ${provider}:`,
          error
        );
        return { success: false, error: error.message };
      }
  ...
  ipcMain.handle(
    IPC_CHANNELS.SETTINGS_DELETE_OAUTH_CREDENTIALS,
    async (event, provider: string) => {
      try {
        if (!settingsService) {
          return { success: false, error: "Settings service not available" };
        ...
        console.error(
          `[IPC] Error deleting OAuth credentials for ${provider}:`,
          error
        );
        return { success: false, error: error.message };
      }
  ...
  ipcMain.handle(
    IPC_CHANNELS.CONNECTOR_START_OAUTH,
    async (event, provider: string, oauthConfig?: any) => {
      try {
        if (!connectorManagerService) {
          return {
            success: false,
        ...
        const authUrl = await connectorManagerService.startOAuthFlow(
          provider as any,
          oauthConfig
        );
  ...
  ipcMain.handle(
    IPC_CHANNELS.CONNECTOR_CONFIGURE_ZOTERO,
    async (event, apiKey: string, userId: string, workspaceId?: string) => {
      try {
        if (!connectorManagerService) {
          return {
            success: false,
        ...
        await connectorManagerService.configureZotero(
          apiKey,
          userId,
          workspaceId
        );
  ...
  ipcMain.handle(
    ...
    async (event, provider: string) => {
      try {
        ...
        const result = await connectorManagerService.testConnector(
          provider as any
        );
        return result;
      } catch (error: any) {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (error.response && error.response.status === 404) {
        console.log(
          "🔧 DEBUG: Dev server responding (404 is expected at root)"
        );
        return true;
      }
      console.log(
        `Dev server not ready yet. Retrying in ${delay}ms... (${i + 1}/${maxRetries})`
      );
  ...
  try {
    ...
    mainWindow.once("ready-to-show", () => {
      ...
      if (process.env.NODE_ENV === "development" || !app.isPackaged) {
        console.log(
          "🔧 DEBUG: Opening DevTools automatically in development mode"
        );
        mainWindow?.webContents.openDevTools({ mode: "detach" });
      }
    });
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      if (serverReady) {
        console.log(
          "🔧 DEBUG: Loading from dev server at http://localhost:3004"
        );
  ...
  const getTrayIcon = (): string | NativeImage => {
    ...
    if (process.platform === "darwin") {
      ...
      const iconPath = path.join(
        process.cwd(),
        "assets/icons/cindy-tray-16.png"
      );
      ...
      try {
        ...
        if (width < 16 || height < 16) {
          ...
          const fallbackPath = path.join(
            process.cwd(),
            "assets/icons/cindy-tray-32.png"
          );
          const fallbackIcon = nativeImage
            .createFromPath(fallbackPath)
...
app.on("ready", async () => {
  ...
  ipcMain.handle(IPC_CHANNELS.GET_DESKTOP_AUDIO_SOURCES, async () => {
    ...
    try {
      ...
      console.log(
        "DEBUG: Main process - source names:",
        sources.map((s) => s.name)
      );
      return sources;
    } catch (error) {
      console.error(
        "DEBUG: Main process - Failed to get desktop audio sources:",
        error
      );
      throw error;
    }
  ...
  try {
    ...
    if (cleanup.processCleanup.killed.length > 0) {
      console.log(
        `✅ Cleaned up ${cleanup.processCleanup.killed.length} stale processes:`,
        cleanup.processCleanup.killed
      );
    }

    if (cleanup.databaseCleanup?.wasLocked) {
      console.log(
        `✅ Database lock cleanup: ${cleanup.databaseCleanup.cleanupSuccessful ? "successful" : "failed"}`
      );
    }
  } catch (cleanupError) {
    console.warn(
      "⚠️  Process cleanup failed (continuing anyway):",
      cleanupError
    );
  }
  ...
  if (!settingsService) {
    try {
      ...
      console.log(
        "🔧 DEBUG: SettingsService singleton initialized successfully"
      );
    } catch (error) {
  ...
  console.log(
    "🔧 DEBUG: Skipping LLM services initialization for startup speed"
  );
  ...
  if (!chatStorageService) {
    try {
      ...
      console.error(
        "🚨 DEBUG: Failed to initialize ChatStorageService:",
        error
      );
  ...
  if (!speechToTextService) {
    try {
      ...
      console.error(
        "🚨 DEBUG: Failed to initialize SpeechToTextService:",
        error
      );
    }
  }
  ...
  if (!textToSpeechService) {
    try {
      ...
      console.log(
        "🔧 DEBUG: Initial TTS provider from settings:",
        selectedProvider
      );
      ...
      if (selectedProvider !== "kokoro") {
        console.warn(
          `🔧 WARNING: Unsupported TTS provider "${selectedProvider}", auto-migrating to kokoro`
        );
        ...
        if (settingsService) {
          try {
            ...
            console.warn(
              "Failed to update TTS provider setting:",
              settingsError
            );
          }
        }
      ...
      console.log(
        "🔧 DEBUG: Final TTS provider for initialization:",
        selectedProvider
      );
      ...
      console.log(
        "🔧 DEBUG: TextToSpeechService created with provider:",
        selectedProvider
      );
    } catch (error) {
  ...
  if (!linkPreviewService) {
    try {
      ...
      console.error(
        "🚨 DEBUG: Failed to initialize LinkPreviewService:",
        error
      );
  ...
  if (!connectorManagerService) {
    try {
      ...
      console.error(
        "🚨 DEBUG: Failed to initialize ConnectorManagerService:",
        error
      );
  ...
  console.log(
    "🔧 DEBUG: ServiceManager initialized for dynamic service loading"
  );
  ...
  ipcMain.handle(IPC_CHANNELS.LLM_GET_AVAILABLE_MODELS, async () => {
    ...
    if (!llmProvider) {
      console.error(
        "Main process - llm:get-available-models: llmRouterService not available"
      );
      return { success: false, error: "LLM Router service not available" };
    }
    try {
      console.log(
        "Main process - llm:get-available-models: calling llmRouterService.getAvailableModels()"
      );
      const models = await llmProvider.getAvailableModels();
      console.log(
        "Main process - llm:get-available-models: successfully retrieved models"
      );
      return { success: true, models };
    } catch (error) {
      console.error(
        "Main process - llm:get-available-models: error retrieving models:",
        error
      );
      return {
        success: false,
  ...
  ipcMain.handle(IPC_CHANNELS.LLM_TEST_CONNECTION, async () => {
    ...
    if (!llmProvider) {
      console.error(
        "Main process - llm:test-connection: llmRouterService not available"
      );
      return {
        success: false,
    ...
    try {
      console.log(
        "Main process - llm:test-connection: testing OpenAI and Ollama connections"
      );
      ...
      console.log(
        "Main process - llm:test-connection: connection results - OpenAI:",
        openaiConnected,
        "Ollama:",
        ollamaConnected
      );
      ...
      console.error(
        "Main process - llm:test-connection: error testing connections:",
        error
      );
      return {
        success: false,
  ...
  ipcMain.handle(
    IPC_CHANNELS.START_FULL_INDEXING,
    async (_, databasePath: string, notesPath?: string) => {
      console.log(
        "[IPC] Full indexing called - Database:",
        databasePath,
        "Notes:",
        notesPath
      );
        ...
        if (!vectorStore) {
          ...
          if (provider === "ollama") {
            ...
            if (openaiApiKey) {
              ...
              vectorStoreConfig.ollamaBaseUrl = "http://localhost:11435";
              console.log(
                "[IPC] No OpenAI API key, falling back to Ollama with smallest Qwen model"
              );
            }
          } else {
            ...
            vectorStoreConfig.ollamaBaseUrl = "http://localhost:11435";
            console.log(
              "[IPC] Using default Ollama embeddings with smallest Qwen model"
            );
          }

          vectorStore = await createDuckDBVectorStore(
            vectorStoreConfig.databasePath,
            vectorStoreConfig,
            app.getPath("userData")
          );
  ...
  ipcMain.handle(
    ...
    async (_, modelName: string) => {
      console.log(
        "Main process - ollama-pull-model IPC called for model:",
        modelName
      );
      try {
        const execAsync = promisify(exec);
        ...
        console.log(
          "Main process - ollama-pull-model: successfully pulled model:",
          modelName
        );
        return { success: true };
      } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (_, modelName: string) => {
      console.log(
        "Main process - ollama-remove-model IPC called for model:",
        modelName
      );
      try {
        const execAsync = promisify(exec);
        ...
        console.log(
          "Main process - ollama-remove-model: successfully removed model:",
          modelName
        );
        return { success: true };
      } catch (error) {
  ...
  ipcMain.handle(IPC_CHANNELS.START_REAL_TIME_TRANSCRIPTION, async () => {
    ...
    try {
      ...
      return {
        success: false,
        error: "Real-time transcription service not available",
      };
    } catch (error) {
      console.error(
        "Main process - start-real-time-transcription error:",
        error
      );
      return {
        success: false,
  ...
  ipcMain.handle(IPC_CHANNELS.STOP_REAL_TIME_TRANSCRIPTION, async () => {
    ...
    try {
      ...
      console.error(
        "Main process - stop-real-time-transcription error:",
        error
      );
      return {
        success: false,
  ...
  ipcMain.handle(IPC_CHANNELS.START_RECORDING, async () => {
    ...
    if (!mainWindow) {
      console.error(
        "DEBUG: Main process - start-recording: mainWindow not available"
      );
      return { success: false, error: "Main window not available" };
    }
    if (mainWindow.webContents.isDestroyed()) {
      console.error(
        "DEBUG: Main process - start-recording: webContents is destroyed"
      );
      return { success: false, error: "Web contents destroyed" };
    }
    try {
      console.log(
        "DEBUG: Main process - start-recording: sending start-recording to renderer"
      );
      mainWindow.webContents.send("start-recording");
      console.log(
        "DEBUG: Main process - start-recording: successfully sent to renderer"
      );
      return { success: true };
    } catch (error) {
      console.error(
        "DEBUG: Main process - start-recording: error sending to renderer:",
        error
      );
      return {
        success: false,
  ...
  ipcMain.handle(IPC_CHANNELS.STOP_RECORDING, async () => {
    ...
    try {
      console.log(
        "Main process - stop-recording: sending get-audio-data to renderer"
      );
      // Send message to renderer to get audio data
      await mainWindow.webContents.send("get-audio-data");
      console.log(
        "Main process - stop-recording: get-audio-data sent to renderer"
      );
      ...
      return new Promise((resolve) => {
        const listener = (
          ...
          console.log(
            "Main process - stop-recording: received audio data from renderer, length:",
            audioData?.length || 0
          );
        ...
        console.log(
          "Main process - stop-recording: listener registered for audio-data"
        );
        ...
        setTimeout(() => {
          ipcMain.removeListener("audio-data", listener);
          console.error(
            "Main process - stop-recording: timeout waiting for audio data"
          );
          resolve(null);
        }, 5000);
      });
      ...
      console.error(
        "Main process - stop-recording: error communicating with renderer:",
        error
      );
      return null;
    }
  ...
  ipcMain.handle(
    ...
    async (event, audioData: Int16Array[] | ArrayBuffer) => {
      console.log(
        "DEBUG: Main process - transcribe-audio IPC called with data type:",
        Array.isArray(audioData) ? "Int16Array[]" : "ArrayBuffer"
      );
      console.log(
        "DEBUG: Main process - transcribe-audio: data size:",
        Array.isArray(audioData)
          ? `${audioData.length} chunks`
          : `${audioData.byteLength} bytes`
      );
      try {
        if (!speechToTextService) {
          console.error(
            "DEBUG: Main process - transcribe-audio: speechToTextService not initialized"
          );
          return "Speech-to-text service not available";
        }

        console.log(
          "DEBUG: Main process - transcribe-audio: calling speechToTextService.transcribe"
        );
        const transcription = await speechToTextService.transcribe(audioData);
        console.log(
          "DEBUG: Main process - transcribe-audio: transcription result:",
          transcription
        );
        return transcription;
      } catch (error) {
        console.error(
          "DEBUG: Main process - transcribe-audio: error transcribing audio:",
          error
        );
        return null;
      }
  ...
  async function initializeLLMServices(): Promise<{
    success: boolean;
    message: string;
  }> {
    console.log("🔧 DEBUG: initializeLLMServices() called");
    console.log(
      "🔧 DEBUG: Current duckDBVectorStore status:",
      !!duckDBVectorStore
    );
    console.log(
      "🔧 DEBUG: Current duckDBVectorStore type:",
      duckDBVectorStore?.constructor?.name
    );
    console.log(
      "🔧 DEBUG: Current duckDBVectorStore has initialize method:",
      typeof duckDBVectorStore?.initialize === "function"
    );
    ...
    try {
      ...
      if (llmProvider) {
        console.log(
          "Main process - LLM provider exists, reinitializing with updated settings"
        );
      ...
      if (
        ...
        console.log(
          "🔧 DEBUG: No API key in keychain, but found in settings - using settings API key"
        );
      ...
      if (
        ...
        if (providerSettings?.apiKey && providerSettings.apiKey !== "***") {
          console.log(
            `🔧 DEBUG: Using API key from ${selectedProvider} settings`
          );
      ...
      console.log(
        `🔧 DEBUG: Provider ${selectedProvider}, API key available: ${apiKey ? "Yes (" + apiKey.length + " chars)" : "No"}`
      );

      if (providersRequiringApiKey.includes(selectedProvider) && !apiKey) {
        return {
          success: false,
          message: `API key is required for ${selectedProvider} provider`,
        };
      }
      ...
      console.log(
        "🔧 DEBUG: Initializing LangChain services with ServiceManager"
      );
      ...
      if (!duckDBVectorStore) {
        try {
          console.log(
            "🔧 DEBUG: Initializing DuckDB vector store for tool registration..."
          );
          ...
          if (!databasePath) {
            ...
            console.log(
              "🔧 DEBUG: No database path configured, using default:",
              databasePath
            );
          }
          ...
          if (provider === "ollama") {
            ...
            vectorStoreConfig.ollamaBaseUrl = "http://localhost:11435";
            console.log(
              "[DEBUG] Using Ollama embeddings for tool registration"
            );
          } else if (provider === "openai") {
            const openaiApiKey = await settingsService?.getApiKey();
            if (openaiApiKey) {
              ...
              vectorStoreConfig.embeddingModel = "text-embedding-3-small";
              console.log(
                "[DEBUG] Using OpenAI embeddings for tool registration"
              );
            } else {
              // Fall back to Ollama if no OpenAI key
              vectorStoreConfig.embeddingProvider = "ollama";
              ...
              vectorStoreConfig.ollamaBaseUrl = "http://localhost:11435";
              console.log(
                "[DEBUG] No OpenAI API key, using Ollama for tool registration"
              );
            }
          } else {
            // Default to Ollama
            ...
            vectorStoreConfig.ollamaBaseUrl = "http://localhost:11435";
            console.log(
              "[DEBUG] Using default Ollama embeddings for tool registration"
            );
          }
          ...
          await duckDBVectorStore.initialize();
          console.log(
            "✅ DEBUG: DuckDB vector store initialized for tool registration"
          );
        } catch (vectorStoreError) {
          console.error(
            "❌ DEBUG: Failed to initialize vector store for tool registration:",
            vectorStoreError
          );
          console.error(
            "❌ DEBUG: Vector store error details:",
            vectorStoreError.message
          );
          console.error(
            "❌ DEBUG: Vector store error stack:",
            vectorStoreError.stack
          );
      ...
      try {
        console.log(
          "🔧 DEBUG: Initializing tools via ServiceManager ToolRegistry"
        );
        console.log(
          "🔧 DEBUG: DuckDB vector store available:",
          !!duckDBVectorStore
        );
        ...
        console.error(
          "❌ DEBUG: Failed to initialize ToolRegistry:",
          toolRegistryError
        );
      ...
      try {
        ...
        console.log(
          "🔧 DEBUG: LLM Config OpenAI API Key:",
          llmConfig.openai?.apiKey
            ? "Yes (" + llmConfig.openai.apiKey.length + " chars)"
            : "No"
        );
        ...
        console.log(
          "🔧 DEBUG: Updating ServiceManager with initialized LLM Provider"
        );
        serviceManager.updateCoreServices(settingsService, llmProvider);
      } catch (llmProviderError) {
        console.error(
          "❌ DEBUG: Failed to initialize LLM Provider:",
          llmProviderError
        );
        return {
          success: false,
          message: `LLM Provider initialization failed: ${llmProviderError.message}`,
        };
      }
      ...
      try {
        if (!langChainCindyAgent && toolRegistry && llmProvider) {
          console.log(
            "🔧 DEBUG: Loading LangChain CindyAgent via ServiceManager"
          );
          langChainCindyAgent =
            await serviceManager.getCindyAgent(duckDBVectorStore);
          console.log("✅ DEBUG: LangChain CindyAgent loaded successfully");
        } else {
          console.warn(
            "⚠️ DEBUG: Skipping Cindy Agent - required services not available"
          );
          console.warn(
            `Tool registry: ${!!toolRegistry}, LLM: ${!!llmProvider}`
          );
        }
      } catch (agentError) {
      ...
      try {
        if (serviceManager && llmProvider) {
          ...
          const tools =
            await serviceManager.getToolsForAgent(duckDBVectorStore);
          console.log(
          ...
          if (modelWithTools) {
            ...
            console.warn(
              "⚠️ DEBUG: Failed to attach tools - model may not support tool binding"
            );
          }
        } else {
          console.warn(
            "⚠️ DEBUG: ServiceManager or LLM provider not available, skipping tool attachment"
          );
        }
      } catch (toolAttachmentError) {
        console.error(
          "❌ DEBUG: Failed to attach tools to LLM:",
          toolAttachmentError
        );
      ...
      return {
        success: true,
        message: `LLM service ${action} successfully with provider: ${llmConfig.provider}`,
      };
    } catch (error) {
  ...
  ipcMain.handle(
    IPC_CHANNELS.UPDATE_LLM_PROVIDER,
    async (event, providerConfig) => {
      console.log(
        "Main process - update-llm-provider IPC called with:",
        providerConfig
      );
      ...
      try {
        ...
        console.log(
          "🔧 DEBUG: Getting existing LLM settings to preserve configurations"
        );
        const existingLlmSettings = await settingsService.get("llm");
        console.log(
          "🔧 DEBUG: Existing LLM settings:",
          Object.keys(existingLlmSettings || {})
        );
        ...
        console.log(
          "🔧 DEBUG: Updating LLM settings with preserved configurations"
        );
        console.log(
          "🔧 DEBUG: Provider switching from",
          existingLlmSettings?.provider,
          "to",
        ...
        if (initResult.success) {
          console.log(
            "✅ DEBUG: Provider switched successfully to:",
            providerConfig.provider
          );
          return {
            success: true,
        ...
        return {
          success: false,
          message: `Provider switch failed: ${error.message}`,
        };
      }
    }
  );
  ...
  ipcMain.handle(
    IPC_CHANNELS.PROCESS_MESSAGE,
    async (event, message: string, conversationId: string): Promise<string> => {
      ...
      if (chatStorageService) {
        try {
          ...
          console.error(
            "🚨 DEBUG: Failed to persist user message at start:",
            saveError
          );
        }
      }
      ...
      try {
        ...
        if (settingsService) {
          ...
          const currentProvider = (currentSettings?.provider ||
            "ollama") as string;
          console.log(
            "🔍 DEBUG: Current provider from settings:",
            currentProvider
          );
          ...
          if (llmProvider) {
            const activeProvider =
              llmProvider.getCurrentProvider() || "unknown";
            console.log(
              `🔍 DEBUG: Provider check - Active: ${activeProvider}, Settings: ${currentProvider}`
            );

            if (activeProvider !== currentProvider) {
              console.log(
                `🔄 Provider mismatch detected! Active: ${activeProvider}, Settings: ${currentProvider}`
              );
              console.log(
                "🔄 Reinitializing LLM provider with current settings..."
              );
              ...
              try {
                ...
                if (currentProvider === "openai") {
                  ...
                  apiKeyForProvider =
                    (currentSettings as any)[currentProvider]?.apiKey || "";
                }
                ...
                if (
                  providersRequiringApiKey.includes(currentProvider) &&
                  !apiKeyForProvider
                ) {
                  console.error(
                    `API key is required for ${currentProvider} provider but not found`
                  );
                } else {
                  ...
                  if (serviceManager) {
                    ...
                    serviceManager.updateCoreServices(
                      settingsService,
                      llmProvider
                    );
                    ...
                    if (toolRegistry) {
                      console.log(
                        "🔄 Reinitializing Cindy Agent with new provider"
                      );
                      ...
                      try {
                        langChainCindyAgent =
                          await serviceManager.getCindyAgent(duckDBVectorStore);
                        console.log(
                          "✅ Cindy Agent reinitialized with new provider"
                        );
                      } catch (error) {
                        console.error(
                          "❌ Failed to reinitialize Cindy Agent:",
                          error
                        );
                      }
                    } else {
                      console.log(
                        "⚠️  Cannot reinitialize Cindy Agent - missing memory or tool services"
                      );
                    }
                  }

                  console.log(
                    "✅ LLM provider reinitialized with:",
                    currentProvider
                  );
                  ...
                  console.log(
                    `📝 Using model: ${modelForProvider} for provider: ${currentProvider}`
                  );
                }
              } catch (error) {
            ...
            console.log(
              "🔍 DEBUG: llmProvider is null, skipping provider check"
            );
          }
        } else {
          console.log(
            "🔍 DEBUG: settingsService is null, skipping provider switching"
          );
        }

        console.log(
          "🔍 DEBUG: After provider switching - langChainCindyAgent:",
          !!langChainCindyAgent,
          "llmProvider:",
          !!llmProvider
        );
        ...
        if (!langChainCindyAgent && !llmProvider) {
          console.error(
            "Main process - process-message: Neither Cindy Agent nor LLM provider initialized"
          );
          ...
          if (settingsService) {
            ...
            try {
              ...
              if (provider === "openai") {
                ...
                provider !== "ollama" &&
                llmSettings &&
                (llmSettings as any)[provider]
              ) {
                apiKeyForProvider =
                  (llmSettings as any)[provider]?.apiKey || "";
              }
              ...
              console.error(
                "Failed to auto-initialize LLM provider:",
                initError
              );
            }
          }
        ...
        if (!langChainCindyAgent && !llmProvider) {
          console.error(
            "Main process - process-message: Still no LLM provider after auto-init attempt"
          );
          ...
          if (provider === "ollama") {
            errorMessage =
              "I'm starting up with Ollama... please wait about 10-15 seconds and try again! ⏳ This only happens on first use.";

            // Trigger background initialization for Ollama
            setImmediate(async () => {
              try {
                console.log(
                  "🔄 Starting background LLM initialization for Ollama..."
                );
            ...
            const apiKey = await settingsService?.getApiKey();
            console.log(
              `🔍 DEBUG: Checking API key for ${provider} provider. Key found: ${apiKey ? "Yes" : "No"}`
            );
            if (!apiKey) {
              // Also check if there's an API key in the settings
              const apiKeyInSettings = llmSettings?.openai?.apiKey;
              console.log(
                `🔍 DEBUG: API key in settings: ${apiKeyInSettings ? "Yes" : "No"}`
              );

              if (!apiKey && !apiKeyInSettings) {
                errorMessage =
                  "Hi! I need an OpenAI API key to chat with you. Please go to Settings → AI PROVIDERS and add your OpenAI API key, then I'll be ready to help! 🤖";
              } else {
                // Try to initialize the LLM provider now
                console.log(
                  "🔄 Attempting to initialize LLM provider with available settings..."
                );
                errorMessage =
                  "I'm initializing my AI services... please wait about 10-15 seconds and try again! ⏳ This only happens on first use.";
              ...
              errorMessage =
                "I'm still initializing my AI services... please wait about 10-15 seconds and try again! ⏳ This only happens on first use.";

              // Trigger background initialization
              setImmediate(async () => {
                try {
                  console.log(
                    "🔄 Starting background LLM initialization for OpenAI/auto..."
                  );
                  const result = await initializeLLMServices();
                  console.log("🔄 Background initialization result:", result);
                } catch (error) {
            ...
            setImmediate(async () => {
              try {
                console.log(
                  `🔄 Starting background LLM initialization for ${provider}...`
                );
                const result = await initializeLLMServices();
                console.log("🔄 Background initialization result:", result);
              } catch (error) {
          ...
          if (chatStorageService) {
            try {
              ...
              console.log(
                "🔧 DEBUG: Error message persisted to ChatStorageService"
              );
            } catch (saveError) {
              console.error(
                "🚨 DEBUG: Failed to persist error message:",
                saveError
              );
            }
          }
          ...
          console.log(
            "🔧 DEBUG: Sending stream-complete event (error path) for conversation:",
            conversationId
          );
        ...
        if (!langChainCindyAgent && llmProvider && serviceManager) {
          try {
            console.log(
              "Main process - attempting to load Cindy Agent dynamically for message processing"
            );
            serviceManager.updateCoreServices(settingsService, llmProvider);
            langChainCindyAgent =
              await serviceManager.getCindyAgent(duckDBVectorStore);
            ...
            console.warn(
              "Main process - failed to load Cindy Agent dynamically:",
              error.message
            );
        ...
        if (langChainCindyAgent) {
          console.log(
            "Main process - using Cindy Agent for intelligent processing"
          );
          console.log(
            "🔍 DEBUG: Cindy Agent LLM provider:",
            langChainCindyAgent["llmProvider"]?.getCurrentProvider()
          );
          ...
          if (chatStorageService) {
            try {
              const history =
                await chatStorageService.getConversationHistory(conversationId);
          ...
          if (agenticMemoryService) {
            try {
              relevantMemories = await agenticMemoryService.retrieveMemories(
                message,
                5
              );
              console.log(
              ...
              agenticMemoryService
                .addMemory(message, conversationId)
                .then((memoryNote) => {
                  console.log(
                    "🧠 DEBUG: User message added to A-Mem:",
                    memoryNote.id
                  );
          ...
          try {
            ...
            const processingStep = generateStepDescription(
              "PROCESSING_REQUEST",
              {
                userQuery: message,
              }
            ...
            for await (const chunk of langChainCindyAgent.processStreaming(
              message,
              agentContext
            )) {
              ...
              while ((toolMatch = toolRegex.exec(chunk)) !== null) {
                try {
                  ...
                  console.log(
                    "🔧 DEBUG: Emitting tool execution update:",
                    toolCallData
                  );

                  // Check if this is a TodoWrite tool execution
                  if (
                    toolCallData.function?.name === "TodoWrite" ||
                    toolCallData.name === "TodoWrite"
                  ) {
                    try {
                      ...
                      if (todos && Array.isArray(todos)) {
                        ...
                        console.log(
                          "📝 DEBUG: Updated global todo list state with",
                          todos.length,
                          "todos"
                        );
                      ...
                      console.error(
                        "🔧 DEBUG: Failed to parse TodoWrite arguments:",
                        todoError
                      );
                    }
                  }
            ...
            console.log(
              "🔧 DEBUG: Sending stream-complete event for conversation:",
              conversationId
            );
            event.sender.send("stream-complete", { conversationId });
            ...
            if (chatStorageService && assistantContent.trim()) {
              try {
                ...
                console.log(
                  "🔧 DEBUG: Assistant message saved to backend storage"
                );
              } catch (saveError) {
                console.error(
                  "🚨 DEBUG: Failed to persist assistant message:",
                  saveError
                );
              }
            }
            ...
            if (agenticMemoryService && assistantContent.trim()) {
              try {
                const memoryNote = await agenticMemoryService.addMemory(
                  assistantContent,
                  conversationId
                );
                console.log(
                  "🧠 DEBUG: Assistant response added to A-Mem:",
                  memoryNote.id
                );
                ...
                console.error(
                  "Failed to add assistant response to memory:",
                  memoryError
                );
              }
            }
            ...
            console.error(
              "Cindy Agent failed, falling back to direct LLM:",
              agentError
            );
        ...
        console.log(
          "Main process - using direct LangChain LLM router as fallback"
        );
        ...
        if (chatStorageService && assistantContent.trim()) {
          try {
            ...
            console.log(
              "🔧 DEBUG: Assistant message persisted to ChatStorageService (cleaned)"
            );
          } catch (saveError) {
            console.error(
              "🚨 DEBUG: Failed to persist assistant message:",
        ...
        console.log(
          "🔧 DEBUG: Sending stream-complete event (fallback path) for conversation:",
          conversationId
        );
        event.sender.send("stream-complete", { conversationId });
        ...
        console.error(
          "Main process - process-message: error processing message:",
          error
        );
  ...
  ipcMain.handle(IPC_CHANNELS.AGENT_MERMAID, async () => {
    ...
    try {
      ...
      if (!llmProvider || !serviceManager) {
        console.log(
          "Main process - agent:mermaid: LLM provider not ready yet, showing initialization diagram"
        );
        return `
graph TD
...
`;
      ...
      try {
        ...
        console.log(
          "Main process - agent:mermaid: Agent not ready yet, error:",
          error.message
        );
        return `
graph TD
...
`;
      ...
      if (!langChainCindyAgent) {
        console.warn(
          "Main process - agent:mermaid: LangGraphAgent not available after initialization attempt"
        );
        return `
graph TD
...
`;
      ...
      const deepResearchIntegration =
        langChainCindyAgent.getDeepResearchIntegration();
      if (deepResearchIntegration) {
        try {
          const deepResearchAgent =
            deepResearchIntegration.getDeepResearchAgent();
          if (deepResearchAgent && deepResearchAgent.getMainGraph) {
            ...
            if (mainGraph && mainGraph.get_graph) {
              ...
              if (graph.drawMermaid) {
                console.log(
                  "Main process - agent:mermaid: Using LangGraph mermaid generation"
                );
          ...
          console.warn(
            "Main process - agent:mermaid: Failed to get LangGraph mermaid, using fallback:",
            graphError.message
          );
        }
      }
      ...
      console.log(
        "Main process - agent:mermaid: Using fallback mermaid generation"
      );
...
`;
      ...
      console.error(
        "Main process - agent:mermaid: Error generating mermaid:",
        error
      );
...
`;
  ...
  ipcMain.handle(IPC_CHANNELS.CREATE_CONVERSATION, async () => {
    ...
    try {
      if (!chatStorageService) {
        console.error(
          "Main process - create-conversation: chatStorageService not available"
        );
  ...
  ipcMain.handle(
    IPC_CHANNELS.LOAD_CONVERSATION,
    async (_, conversationId: string) => {
      console.log(
        "Main process - load-conversation IPC called for:",
        conversationId
      );
      try {
        if (!chatStorageService) {
          console.error(
            "Main process - load-conversation: chatStorageService not available"
          );
          return [];
        }
        ...
        console.error(
          "Main process - load-conversation: error loading conversation:",
          error
        );
        return [];
      }
  ...
  ipcMain.handle(IPC_CHANNELS.MEMORY_GRAPH_GET_DATA, async () => {
    ...
    try {
      if (!agenticMemoryService) {
        ...
        if (llmProvider) {
          const { AgenticMemoryService } = await import(
            "./services/AgenticMemoryService"
          );
  ...
  ipcMain.handle(
    IPC_CHANNELS.MEMORY_GRAPH_ADD_MEMORY,
    async (_, content: string, conversationId?: string) => {
      ...
      try {
        ...
        const memory = await agenticMemoryService.addMemory(
          content,
          conversationId
        );
        ...
        if (mainWindow) {
          mainWindow.webContents.send(
            ...
            await agenticMemoryService.getMemoryGraphData()
          );
        }
  ...
  ipcMain.handle(
    IPC_CHANNELS.MEMORY_GRAPH_RETRIEVE,
    async (_, query: string, limit: number = 10) => {
      ...
      try {
        ...
        const memories = await agenticMemoryService.retrieveMemories(
          query,
          limit
        );
  ...
  ipcMain.handle(
    IPC_CHANNELS.LOAD_ALL_CONVERSATION_MESSAGES,
    async (_, conversationId: string) => {
      console.log(
        "Main process - load-all-conversation-messages IPC called for:",
        conversationId
      );
      try {
        if (!chatStorageService) {
          console.error(
            "Main process - load-all-conversation-messages: chatStorageService not available"
          );
          return [];
        }
        const allMessages =
          await chatStorageService.getAllConversationMessages(conversationId);
        console.log(
          "Main process - Returning",
          allMessages.length,
          "unfiltered messages"
        );
        return allMessages;
        ...
        console.error(
          "Main process - load-all-conversation-messages: error loading messages:",
          error
        );
        return [];
      }
  ...
  ipcMain.handle(IPC_CHANNELS.GET_INCOMPLETE_CONVERSATIONS, async () => {
    ...
    try {
      if (!chatStorageService) {
        console.error(
          "Main process - get-incomplete-conversations: chatStorageService not available"
        );
        return [];
      }
      ...
      console.error(
        "Main process - get-incomplete-conversations: error:",
        error
      );
      return [];
    }
  ...
  ipcMain.handle(
    IPC_CHANNELS.GET_CONVERSATION_HEALTH,
    async (_, conversationId: string) => {
      console.log(
        "Main process - get-conversation-health IPC called for:",
        conversationId
      );
      try {
        if (!chatStorageService) {
          console.error(
            "Main process - get-conversation-health: chatStorageService not available"
          );
          return null;
        }
  ...
  ipcMain.handle(
    IPC_CHANNELS.GET_THINKING_BLOCKS,
    async (_, conversationId: string) => {
      console.log(
        "Main process - get-thinking-blocks IPC called for:",
        conversationId
      );
      try {
        if (!chatStorageService) {
          console.error(
            "Main process - get-thinking-blocks: chatStorageService not available"
          );
          return [];
        }
        ...
        if (typeof chatStorageService.getThinkingBlocks === "function") {
          ...
          console.warn(
            "Main process - get-thinking-blocks: method not implemented yet"
          );
          return [];
        }
      } catch (error) {
        console.error(
          "Main process - get-thinking-blocks: error loading thinking blocks:",
          error
        );
        return [];
      }
  ...
  ipcMain.handle(IPC_CHANNELS.GET_CONVERSATIONS, async () => {
    ...
    try {
      if (!chatStorageService) {
        console.error(
          "Main process - get-conversations: chatStorageService not available"
        );
        return [];
      }
  ...
  ipcMain.handle(
    IPC_CHANNELS.GET_LATEST_HUMAN_MESSAGE,
    async (_, conversationId: string) => {
      console.log(
        "Main process - get-latest-human-message IPC called for:",
        conversationId
      );
      try {
        if (!chatStorageService) {
          console.error(
            "Main process - get-latest-human-message: chatStorageService not available"
          );
          return null;
        }
        ...
        console.error(
          "Main process - get-latest-human-message: error getting latest human message:",
          error
        );
        return null;
      }
  ...
  ipcMain.handle(IPC_CHANNELS.SAVE_MESSAGE, async (event, messageData) => {
    console.log(
      "🔧 DEBUG: Main process - save-message IPC called with:",
      messageData
    );
    try {
      if (!chatStorageService) {
        console.error(
          "🚨 DEBUG: Main process - save-message: chatStorageService not available"
        );
      ...
      console.log(
        "🔧 DEBUG: Main process - save-message: message saved with ID:",
        messageId
      );
      return { success: true, messageId };
    } catch (error) {
      console.error(
        "🚨 DEBUG: Main process - save-message: error saving message:",
        error
      );
      return {
        success: false,
  ...
  console.log(
    "🔧 DEBUG: Creating window and tray after all services are ready"
  );
  await createWindow();
  await createTray();
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    console.log(
      "🔧 DEBUG: Waiting for window to be fully loaded before setting global flag"
    );
    ...
    try {
      ...
      console.error(
        "🚨 DEBUG: Failed to set global settings service flag:",
        error
      );
    }
  }
      ...
      try {
        ...
        if (settings) {
          ...
          if (provider === "ollama") {
            ...
            if (!apiKey) {
              console.log(
                "⚠️  DEBUG: OpenAI/auto provider requires API key, user will need to add one in settings"
              );
            }
          }
