...
export class DuckDBVectorStore extends EventEmitter {
  ...
  private getProviderDefaults(
    provider: string
  ): Partial<DuckDBVectorStoreConfig> {
    switch (provider) {
      case "huggingface":
        return {
  ...
  private createEmbeddingsProvider(): Embeddings {
    ...
    switch (provider) {
      case "ollama":
        console.log(
          "[DuckDBVectorStore] Using Ollama embeddings:",
          this.config.embeddingModel
        );
        if (this.config.ollamaBaseUrl?.includes("localhost")) {
          this.config.ollamaBaseUrl = this.config.ollamaBaseUrl.replace(
            "localhost",
            "127.0.0.1"
          );
          console.log(
            "[DuckDBVectorStore] Normalized Ollama base URL to IPv4:",
            this.config.ollamaBaseUrl
          );
        }
        console.log(
          "[DuckDBVectorStore] Ollama base URL:",
          this.config.ollamaBaseUrl
        );
      ...
      case "huggingface":
        console.log(
          "[DuckDBVectorStore] Using HuggingFace local embeddings:",
      ...
      case "openai":
        if (!this.config.openaiApiKey) {
          throw new Error(
            "OpenAI API key is required when using OpenAI embeddings provider"
          );
        }
        console.log(
          "[DuckDBVectorStore] Using OpenAI embeddings (Ada):",
          this.config.embeddingModel
        );
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      "[DuckDBVectorStore] Initializing vector store at path:",
    ...
    try {
      ...
      console.log(
        "[DuckDBVectorStore] Creating in-memory DB to load extensions before attaching file DB"
      );
      this.db = await Database.create(":memory:");

      console.log(
        "[DuckDBVectorStore] Installing and loading VSS extension..."
      );
      ...
      console.log(
        "[DuckDBVectorStore] Attaching on-disk database:",
        this.config.databasePath
      );
      await this.db.all(
        `ATTACH '${this.config.databasePath}' AS diskdb (READ_WRITE);`
      );
      await this.db.all(`USE diskdb;`);

      // After attaching, ensure VSS extension is also loaded for the attached database
      console.log(
        "[DuckDBVectorStore] Loading VSS extension on attached database..."
      );
      try {
        await this.db.all(`LOAD vss;`);
        console.log(
          "[DuckDBVectorStore] Verified VSS extension loaded on attached DB"
        );
        // const extList = await this.db.all(`SELECT * FROM duckdb_extensions();`);
        // console.log('[DuckDBVectorStore] Active extensions:', extList);
      } catch (extErr) {
        console.error(
          "[DuckDBVectorStore] Failed to load VSS on attached DB:",
          extErr
        );
      }
      ...
      try {
        await this.db.all(
          `CREATE TABLE IF NOT EXISTS _vss_test (id INTEGER, embedding FLOAT[${this.config.vectorDimension}]);`
        );
        await this.db.all(`CREATE INDEX IF NOT EXISTS idx_vss_test_embedding
                    ON _vss_test USING HNSW (embedding)
                    WITH (metric = 'cosine')`);
        console.log(
          "[DuckDBVectorStore] Verified HNSW index creation is possible on attached DB"
        );
      } catch (verifyErr) {
        console.error(
          "[DuckDBVectorStore] HNSW index creation test failed:",
          verifyErr
        );
      }
  ...
  private async createTables(): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(testVec) && testVec.length > 0) {
        ...
        console.log(
          `[DuckDBVectorStore] Detected embedding dimension: ${embedDim}`
        );
      }
    } catch (e) {
      console.warn(
        "[DuckDBVectorStore] Failed to auto-detect embedding dimension, using config default:",
        embedDim
      );
    }
    ...
    try {
      ...
      console.log(
        `[DuckDBVectorStore] 📋 Existing documents table schema:`,
        tableInfo
      );

      // Find the embedding column and check its dimension
      const embeddingColumn = tableInfo.find(
        (col: any) => col.column_name === "embedding"
      );
      if (embeddingColumn) {
        const existingDimMatch =
          embeddingColumn.column_type.match(/FLOAT\[(\d+)\]/);
        const existingDim = existingDimMatch
          ? parseInt(existingDimMatch[1])
          : null;

        if (existingDim !== embedDim) {
          console.log(
            `[DuckDBVectorStore] ⚠️ Dimension mismatch: existing=${existingDim}, required=${embedDim}. Table recreation needed.`
          );
          needsRecreation = true;
        } else {
          console.log(
            `[DuckDBVectorStore] ✅ Documents table exists with correct dimension (${embedDim}). Preserving existing data.`
          );
        }
      } else {
        console.log(
          `[DuckDBVectorStore] ⚠️ Documents table exists but no embedding column found. Recreation needed.`
        );
        needsRecreation = true;
      }
    } catch (error) {
      ...
      console.log(
        `[DuckDBVectorStore] 📝 Documents table doesn't exist, creating new one...`
      );
      needsRecreation = true;
    }
    ...
    try {
      ...
      console.warn(
        "[DuckDBVectorStore] HNSW index creation failed, using slower brute force search:",
        error.message
      );
  ...
  async addDocuments(documents: Document[]): Promise<void> {
    ...
    try {
      console.log(
        "[DuckDBVectorStore] Embedding documents using provider:",
        this.config.embeddingProvider
      );
      console.log(
        "[DuckDBVectorStore] Embedding model:",
        this.config.embeddingModel
      );
      if (this.config.embeddingProvider === "ollama") {
        console.log(
          "[DuckDBVectorStore] Ollama base URL (before embedDocuments):",
          this.config.ollamaBaseUrl
        );
      }
      ...
      if (!embeddings || embeddings.length === 0) {
        console.error(
          "[DuckDBVectorStore] No embeddings returned for provided documents"
        );
        return;
      }
      const embedDim =
        embeddings[0]?.length || this.config.vectorDimension || 0;

      console.log(
        `[DuckDBVectorStore] Preparing INSERT statement for ${embedDim} dimensions`
      );
      ...
      for (let i = 0; i < documents.length; i++) {
        ...
        if (!embedding || embedding.length === 0) {
          console.warn(
            `[DuckDBVectorStore] Skipping document ${id} due to empty embedding`
          );
          continue;
        }
        ...
        try {
          ...
          console.warn(
            `[DuckDBVectorStore] Failed to serialize metadata for ${id}, using empty object`,
            metaErr
          );
          metadataStr = "{}";
        }
        ...
        try {
          ...
          const safeMetadataStr = Buffer.from(metadataStr, "utf8").toString(
            "utf8"
          );
          ...
          if (embeddingPlain.length !== embedDim) {
            console.warn(
              `[DuckDBVectorStore] Embedding length mismatch for ${id}: got ${embeddingPlain.length}, expected ${embedDim}`
            );
          }
          ...
          console.log(
            `[DuckDBVectorStore] 🔍 Inserting document ${id}: content=${safeContent.length} chars, embedding=${embeddingPlain.length} dims`
          );
          ...
          if (i === 0) {
            // Only log for first document to avoid spam
            const justInserted = await this.db.all(
              "SELECT COUNT(*) as count FROM documents WHERE id = ?",
              [id]
            );
            console.log(
              `[DuckDBVectorStore] 🔍 Verification: document ${id} exists in table: ${justInserted[0]?.count === 1 ? "YES" : "NO"}`
            );
          }
        } catch (err) {
          console.error(
            `[DuckDBVectorStore] Failed to insert document ${id}:`,
            err
          );
          if (
            err instanceof Error &&
            /Invalid byte encountered/.test(err.message)
          ) {
            console.error(
              `[DuckDBVectorStore] Detected encoding error for document ${id}, attempting to hex-escape non-ASCII characters and retry`
            );
            try {
              const escapedContent = content.replace(/[^\x00-\x7F]/g, (ch) => {
                return "\\x" + Buffer.from(ch).toString("hex").toUpperCase();
              });
              const escapedMetadata = metadataStr.replace(
                /[^\x00-\x7F]/g,
                (ch) => {
                  return "\\x" + Buffer.from(ch).toString("hex").toUpperCase();
                }
              ...
              await stmt.run(
                ...
                escapedContent,
                escapedMetadata,
                retryEmbeddingString
              );
              console.log(
                `[DuckDBVectorStore] Successfully retried document ${id} with escaped characters`
              );
              successCount++;
            } catch (retryErr) {
              console.error(
                `[DuckDBVectorStore] Retry failed for document ${id}:`,
                retryErr
              );
            }
          }
      ...
      console.log(
        `[DuckDBVectorStore] Added ${successCount} of ${documents.length} documents successfully`
      );

      // Verify documents were actually inserted
      try {
        const verifyCount = await this.db.all(
          "SELECT COUNT(*) as count FROM documents"
        );
        console.log(
          `[DuckDBVectorStore] 🔍 Verification: documents table now has ${verifyCount[0]?.count || 0} total rows`
        );

        if (verifyCount[0]?.count > 0) {
          const sampleDoc = await this.db.all(
            "SELECT LEFT(content, 100) as preview FROM documents LIMIT 1"
          );
          console.log(
            `[DuckDBVectorStore] 🔍 Sample document content: "${sampleDoc[0]?.preview}..."`
          );
        } else {
          console.error(
            `[DuckDBVectorStore] ❌ CRITICAL: Documents table is empty after claiming to add ${successCount} documents!`
          );
        }
      } catch (verifyError) {
        console.error(
          `[DuckDBVectorStore] ❌ Failed to verify document insertion:`,
          verifyError
        );
      }
    } catch (error) {
      ...
      if (stmt) {
        try {
          ...
          console.error(
            "[DuckDBVectorStore] Error finalizing statement:",
            finalizeErr
          );
        }
      }
  ...
  async similaritySearch(query: string, k: number = 5): Promise<Document[]> {
    ...
    console.log(
      `[DuckDBVectorStore] Starting similarity search for: "${query}" (k=${k})`
    );
    ...
    console.log(
      `[DuckDBVectorStore] 🔍 Creating embedding for query: "${query}"`
    );
    console.log(
      `[DuckDBVectorStore] 🔍 Embedding provider: ${this.config.embeddingProvider}`
    );
    console.log(
      `[DuckDBVectorStore] 🔍 Embedding model: ${this.config.embeddingModel}`
    );
    ...
    try {
      ...
      console.log(
        `[DuckDBVectorStore] ✅ Query embedding created successfully`
      );
      console.log(
        `[DuckDBVectorStore] 📊 Query embedding dimension: ${queryEmbedding.length}`
      );
      console.log(
        `[DuckDBVectorStore] 📊 Query embedding sample (first 5): [${queryEmbedding
          .slice(0, 5)
          .map((n) => n.toFixed(4))
          .join(", ")}...]`
      );
      console.log(
        `[DuckDBVectorStore] 📊 Query embedding range: min=${Math.min(...queryEmbedding).toFixed(4)}, max=${Math.max(...queryEmbedding).toFixed(4)}`
      );
    } catch (embeddingError) {
      console.error(
        `[DuckDBVectorStore] ❌ Failed to create query embedding:`,
        embeddingError
      );
    ...
    try {
      ...
      console.log(
        "[DuckDBVectorStore] Could not query available functions:",
        funcError.message
      );
    }
    ...
    const searchMethods = [
      {
        ...
        query: `
                    SELECT 
                        content,
                        metadata,
                        array_cosine_distance(embedding, ${vectorLiteral}::FLOAT[${queryDimension}]) as distance
                    FROM documents
    ...
    for (const method of searchMethods) {
      try {
        console.log(
          `[DuckDBVectorStore] 🔍 Trying similarity search method: ${method.name}`
        );
        console.log(
          `[DuckDBVectorStore] 🔍 Query: ${method.query.replace(/\[[\d\.,\-\s]+\]/g, "[EMBEDDING_VECTOR]")}`
        );

        const results = await this.db.all(method.query);

        console.log(
          `[DuckDBVectorStore] ✅ ${method.name} executed successfully!`
        );
        console.log(
          `[DuckDBVectorStore] 📊 Raw results count: ${results.length}`
        );
        ...
        if (results.length > 0) {
          ...
          console.log(
            `[DuckDBVectorStore] ⚠️ ${method.name} returned 0 results - trying next method`
          );
          continue;
        }
      } catch (methodError) {
        console.log(
          `[DuckDBVectorStore] ❌ ${method.name} failed:`,
          methodError.message
        );
        continue;
      }
    ...
    console.log(
      "[DuckDBVectorStore] 🔍 All vector similarity methods failed, falling back to text search"
    );
    try {
      const searchPattern = `%${query.toLowerCase()}%`;
      console.log(
        "[DuckDBVectorStore] 🔍 Fallback search pattern:",
        searchPattern,
        "limit:",
        k
      );
      ...
      console.log(
        "[DuckDBVectorStore] 🔍 Text search query:",
        textSearchQuery.replace(/\n\s+/g, " ")
      );
      const results = await this.db.all(textSearchQuery);

      console.log(
        `[DuckDBVectorStore] 📊 Text search found ${results.length} results`
      );
      ...
      if (results.length > 0) {
        ...
        console.log(
          `[DuckDBVectorStore] ⚠️ Text search also returned 0 results`
        );

        // Final diagnostic: check if there's ANY content in the database
        const anyContent = await this.db.all(
          "SELECT content FROM documents LIMIT 1"
        );
        if (anyContent.length > 0) {
          console.log(
            `[DuckDBVectorStore] 🔍 Database has content, but search failed. Sample content:`,
            anyContent[0].content.substring(0, 200) + "..."
          );
        } else {
          console.log(
            `[DuckDBVectorStore] ❌ Database documents table is completely empty!`
          );
        }
      }
      ...
      console.error(
        "[DuckDBVectorStore] ❌ All search methods failed:",
        fallbackError
      );
      return [];
    }
  ...
  async indexFolder(
    ...
    console.log(
      `[DuckDBVectorStore] Starting to index ${totalFiles} files from ${folderPath}`
    );

    if (totalFiles === 0) {
      console.log(
        "[DuckDBVectorStore] No supported files found. Supported extensions: .pdf, .txt, .md, .json, .docx, .doc"
      );
      return stats;
    }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      console.log(
        `[DuckDBVectorStore] Progress: ${progressData.current}/${progressData.total} (${progressData.percentage}%) - ${file}`
      );
    ...
    console.log(
      `[DuckDBVectorStore] Indexing complete. Success: ${stats.success}, Errors: ${stats.errors}`
    );
    ...
    try {
      ...
      switch (ext) {
        ...
        default:
          ...
          console.log(
            `[DuckDBVectorStore] Skipping unsupported file type: ${ext}`
          );
          return;
      }
      ...
      console.error(
        `[DuckDBVectorStore] Error indexing file ${filePath}:`,
        error
      );
  ...
  private async saveIndexedFileInfo(fileInfo: IndexedFile): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[DuckDBVectorStore] Saved indexed file info for ${fileInfo.name}`
      );
    } catch (error) {
      console.error(
        "[DuckDBVectorStore] Error saving indexed file info:",
        error
      );
  ...
  private getAllFiles(dirPath: string, arrayOfFiles: string[] = []): string[] {
    ...
    try {
      ...
      console.log(
        `[DuckDBVectorStore] Scanning directory: ${dirPath} (${files.length} items)`
      );
      ...
      files.forEach((file) => {
        ...
        try {
          ...
          if (stat.isDirectory()) {
            ...
            if (file.startsWith(".")) {
              console.log(
                `[DuckDBVectorStore] Skipping hidden directory: ${file}`
              );
              return;
            }
            ...
            if (
              ...
              console.log(
                `[DuckDBVectorStore] Skipping excluded directory: ${file}`
              );
              return;
            }
            ...
            console.log(
              `[DuckDBVectorStore] File found: ${file} (extension: ${ext})`
            );

            if (supportedExtensions.has(ext)) {
              arrayOfFiles.push(filePath);
              console.log(
                `[DuckDBVectorStore] ✅ Added supported file: ${filePath}`
              );
            } else {
              console.log(
                `[DuckDBVectorStore] ❌ Unsupported extension: ${ext} for file ${file}`
              );
            }
          }
        } catch (error) {
          console.warn(
            `[DuckDBVectorStore] Error accessing ${filePath}:`,
            error.message
          );
        }
      });
      ...
      console.error(
        `[DuckDBVectorStore] Error reading directory ${dirPath}:`,
        error.message
      );
    }

    console.log(
      `[DuckDBVectorStore] Total supported files found: ${arrayOfFiles.length}`
    );
    return arrayOfFiles;
  }
  ...
  async debugDatabaseContents(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "[DuckDBVectorStore] 📊 Indexed files count:",
        indexedFiles[0]?.count || 0
      );

      if (indexedFiles[0]?.count > 0) {
        const sampleFiles = await this.db.all(
          "SELECT file_name, chunk_count FROM indexed_files LIMIT 5"
        );
        console.log(
          "[DuckDBVectorStore] 📋 Sample indexed files:",
          sampleFiles
        );
      }

      // Check documents table
      const documentsCount = await this.db.all(
        "SELECT COUNT(*) as count FROM documents"
      );
      console.log(
        "[DuckDBVectorStore] 📊 Document chunks count:",
        documentsCount[0]?.count || 0
      );

      if (documentsCount[0]?.count > 0) {
        const sampleDocs = await this.db.all(
          "SELECT LEFT(content, 100) as preview, metadata FROM documents LIMIT 3"
        );
        console.log(
          "[DuckDBVectorStore] 📋 Sample document chunks:",
          sampleDocs.map((doc) => ({
            preview: doc.preview + "...",
      ...
      const embeddingInfo = await this.db.all(
        "SELECT array_length(embedding) as dim_count FROM documents LIMIT 1"
      );
      if (embeddingInfo.length > 0) {
        console.log(
          "[DuckDBVectorStore] 📊 Embedding dimensions:",
          embeddingInfo[0]?.dim_count || "unknown"
        );
      }
    } catch (error) {
      console.error(
        "[DuckDBVectorStore] ❌ Error checking database contents:",
        error
      );
    }
  }
  ...
  async checkDirectoryStatus(folderPath: string): Promise<{
    ...
    const indexedCount = indexedFiles.filter((f) =>
      currentFiles.includes(f.path)
    ).length;
...
export async function createDuckDBVectorStore(
  ...
  console.log(
    "[IPC] Vector database will be stored at:",
