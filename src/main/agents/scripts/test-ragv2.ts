...
async function initializeAgent(): Promise<MainAgentExecution> {
  ...
  console.log(
    `✅ LLM Provider initialized with: ${llmProvider.getCurrentProvider()}\n`
  );
  ...
  console.log(
    "✅ Memory Service created (skipping initialization for testing)\n"
  );
  ...
  const databasePath =
    "/Users/karwo09/code/voice-assistant/data/test-vectorstore";
  const vectorStore = await createDuckDBVectorStore(
    databasePath,
    llmOptions,
    "/Users/karwo09/code/voice-assistant/data/appDataPathTest"
  ); // Create a mock or in-memory vector store for testing
...
if (require.main === module) {
  ...
  console.log(
    "║          RouterLangGraphAgent Test Suite                      ║"
  );
  console.log(
    "╚═══════════════════════════════════════════════════════════════╝"
  );
